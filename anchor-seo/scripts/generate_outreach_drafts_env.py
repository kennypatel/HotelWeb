#!/usr/bin/env python3
"""
generate_outreach_drafts_env.py — build-environment COPY of generate_outreach_drafts.py.

Only difference from the original: this sandbox's egress proxy blocks all external
hosts, so Phase 4 produced Verdict=UNVERIFIED (policy-blocked, NOT dead) instead of
LIVE. The original script keeps only Verdict == "LIVE" rows, which would yield zero
drafts here. This copy keeps every row that is not DEAD/DEAD? (i.e. LIVE, UNVERIFIED,
REVIEW). Everything else — placeholder, caps, output format — is identical.
...
def main():
    ...
    # env change: exclude only confirmed-dead rows; UNVERIFIED = policy-blocked, not dead
    rows = [r for r in rows if r.get("Verdict", "LIVE") not in ("DEAD", "DEAD?")]
